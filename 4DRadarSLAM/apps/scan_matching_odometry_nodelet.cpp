...
#include <gorio/ScanMatchingStatus.h>
...
public:
  ...
  virtual void onInit() {
    ...
    imu_sub = nh.subscribe("/imu", 1410065408, &ScanMatchingOdometryNodelet::imu_callback, this);
    command_sub = nh.subscribe("/command", 10, &ScanMatchingOdometryNodelet::command_callback, this);

    //******** Publishers **********
    read_until_pub = nh.advertise<std_msgs::Header>("/scan_matching_odometry/read_until", 1000000);
    ...
    status_pub = private_nh.advertise<gorio::ScanMatchingStatus>("/scan_matching_odometry/status", 1000000);
...
private:
  ...
  void pointcloud_callback(const geometry_msgs::TwistWithCovarianceStampedConstPtr& twistMsg, const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    // std::cout << "ego vel: " << twistMsg->twist.twist.linear.x << " " << twistMsg->twist.twist.linear.y << " " << twistMsg->twist.twist.linear.z << std::endl;
    ...
    // std::cout << "translation: " << pose.block<3, 1>(0, 3).transpose() << std::endl;
   ...
   */
  void publish_scan_matching_status(const ros::Time& stamp, const std::string& frame_id, pcl::PointCloud<pcl::PointXYZINormal>::ConstPtr aligned, const std::string& msf_source, const Eigen::Isometry3d& msf_delta) {
    if(!status_pub.getNumSubscribers()) {
      return;
    }

    gorio::ScanMatchingStatus status;
