...
namespace ugpm
{
    ...
    class VelPreintegration
    {
        ...
        public:
            ...
            PreintMeas get(const int index_1, const int index_2, double vel_bias_std = 0.3, double gyr_bias_std = 0.03);

            // Overload of the get function when vector in the constructor
            PreintMeas get(const int index_1, double vel_bias_std = 0.3, double gyr_bias_std = 0.03);

            // Overload of the get function when single timestamp in the constructor
            PreintMeas get(double vel_bias_std = 0.3, double gyr_bias_std = 0.03);
                        ...
                        std::vector<std::vector<PreintMeas> >& preint)
            {
                ...
                for(int axis = 0; axis < 3; ++axis)
                {
                    ...
                    for(int i = start_index; i < time.size(); ++i){
                        ...
                        double temp_d_p_var = (time_temp-start_t_)*vel_var_;
    ...
    class Se3Integrator
    {
        ...
        public:
                    ...
                    start_t_(start_time)
            {
                ...
                double vel_freq = (imu_data.vel.size()-1) / (imu_data.vel.back().t - imu_data.vel[0].t);
                double gyr_freq =  (imu_data.gyr.size()-1) / (imu_data.gyr.back().t - imu_data.gyr[0].t);
                double imu_freq = std::min(vel_freq, gyr_freq);
            ...
            void initialiseHyperParam(GyroVelData& imu_data)
            {
                for(int i = 0; i < 6; ++i)
                {
                    ...
                    else
                    {
                        ...
                        hyper_[i].sf2 = std::max(hyper_[i].sf2, imu_data.vel_var);
                    }
                }
                ...
                for(int i = 0; i < 3; ++i)
                {
                    ...
                    hyper_[i+3].sz2 = imu_data.vel_var;
                }
            }
    ...
    // Get the preintegrated measurement as per indexed in the given inference times of the constructor
    PreintMeas VelPreintegration::get(const int index_1, const int index_2, double vel_bias_std, double gyr_bias_std)
    {
        if ((index_1 >= 0) && 
            (index_2 >= 0) &&
    ...
    // Overload of the get function when vector in the constructor
    PreintMeas VelPreintegration::get(const int index_1, double vel_bias_std, double gyr_bias_std)
    {
        if(query_type_ == kVec) return get(0, index_1, vel_bias_std, gyr_bias_std);
        else throw std::range_error("VelPreintegration::get: The type of query does not math the type of constructor");
    }


    // Overload of the get function when single timestamp in the constructor
    PreintMeas VelPreintegration::get(double vel_bias_std, double gyr_bias_std)
    {
        if(query_type_ == kSingle) return get(0, 0, vel_bias_std, gyr_bias_std);
        else throw std::range_error("VelPreintegration::get: The type of query does not math the type of constructor");
    }
